...
def cfg_train(cfg_train_global, tmp_path) -> DictConfig:
    ...
    # this is a workaround for infinite datasets from simulation,
    # IterableDataset. we first check if dataset corresponding to the
    # dataloader has base class IterableDataset, and then set the
    # trainer flags properly.
    try:
        dset = instantiate(cfg.datamodule.dataset)
...
def cfg_eval(cfg_eval_global, tmp_path) -> DictConfig:
    ...
    # same workaround for IterableDataset as above!
    try:
        dset = instantiate(cfg.datamodule.dataset)
        if dset.__class__.__base__.__name__ == "IterableDataset":
            cfg.trainer.limit_test_batches = 5
