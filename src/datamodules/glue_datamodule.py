...
class QNLIDataModule(GLUEDataModuleBase):
    ...
    def _tokenize(self, dataset) -> torch.utils.data.TensorDataset:
        ...
        """
        tokenizer = AutoTokenizer.from_pretrained(self.model_type)
        # Note: QNLI has both 'question' and 'sentence' fields that need to be tokenized together.
...
class MNLIDataModule(GLUEDataModuleBase):
    ...
    def setup(self, stage: str = None) -> None:
        ...
        # Note: MNLI also has a 'validation_mismatched' split. You might want to include that as well.
    ...
    def _tokenize(self, dataset) -> torch.utils.data.TensorDataset:
        ...
        # labels = torch.tensor([label_map[label] for label in dataset["label"]], dtype=torch.long)
        labels = torch.tensor(dataset["label"], dtype=torch.long)
