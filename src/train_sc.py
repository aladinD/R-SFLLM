...
import utils
from datamodules.glue_datamodule import SST2DataModule, MRPCDataModule, QNLIDataModule, MNLIDataModule
from models.bert_module import BERTForSequenceClassificationModule
from models.roberta_module import RoBERTaForSequenceClassificationModule
...
@hydra.main(version_base="1.3", config_path=".", config_name="config_sc")
def main(cfg):
    ...
    if cfg.model.config.pretrained_model_name_or_path == "bert-base-uncased":
        model = BERTForSequenceClassificationModule.from_pretrained(**cfg.model.config)
    elif cfg.model.config.pretrained_model_name_or_path == "roberta-base":
        model = RoBERTaForSequenceClassificationModule.from_pretrained(**cfg.model.config)
    else:
